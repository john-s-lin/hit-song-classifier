...
DOI citation: Dhruvil Dave. (2021). <i>Billboard "The Hot 100" Songs</i> [Data set]. Kaggle. https://doi.org/10.34740/KAGGLE/DS/1211465
'''
...
def download_kaggle_dataset(k_path: str, k_filename: str, directory: str):
    ...
    with ZipFile(f'{os.path.join(directory, k_filename)}.zip', 'r') as zip_object:
...
def write_csv(df: DataFrame, write_path: str):
    ...
    for file in os.listdir(write_path):
        if file.endswith(".csv"):
            os.rename(os.path.join(write_path, file), os.path.join(root_dir, 'data', f'charts{count}.csv'))
            count += 1
...
write_csv(classified_rankings, os.path.join(root_dir, 'data', 'classified_songs'))
